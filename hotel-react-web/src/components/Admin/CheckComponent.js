...
class CheckComponent extends Component {
    ...
    componentDidMount() {
        ...
        fetch('http://localhost:3001/check', requestOptions)
    ...
    search() {
        ...
        fetch('http://localhost:3001/check' + condition, requestOptions)
            .then(response => response.json())
            .then(data => {
                this.setState({ 'book': data });
            })
        ...
        return (
            <div className="bg-div" style={{ 'paddingLeft': '2%', 'paddingRight': '2%' }}>
                <div className="header-topic">
                    <span className="header-reserve">BOOKING CHECK-IN/CHECK-OUT</span>
                    <hr />
                </div>
                <div className="row">
                        ...
                        <tbody>
                            {this.state.book.map((room, index) => (
                                <tr key={index} style={{ 'verticalAlign': 'middle' }}>
                                    ...
                                    <td style={{ 'textAlign': 'center' }}>{room.cIntime !== null ? moment(room.cIntime).format('DD-MM-YYYY HH:mm') : <a href={"/check-in?bookingid=" + room.BookingID}><button className="btn btn-success">Check-in</button></a>}</td>
                                    <td style={{ 'textAlign': 'center' }}>{room.cOuttime !== null ? moment(room.cOuttime).format('DD-MM-YYYY HH:mm') : room.cIntime != null ? <a href={"/check-out?bookingid=" + room.BookingID}><button className="btn btn-success">Check-out</button></a> : '-'}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
