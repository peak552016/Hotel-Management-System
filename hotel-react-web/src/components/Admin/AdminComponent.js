...
class AdminComponent extends Component {
    ...
    search() {
        ...
        fetch('http://localhost:3001/admin' + condition, requestOptions)
            .then(response => response.json())
            .then(data => {
                this.setState({ 'book': data });
            })
                    ...
                    <table className="table">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" style={{ 'textAlign': 'center' }}>Reason Cancel</th>
                        ...
                        <tbody>
                            {this.state.book.map((room, index) => (
                                <tr key={index} style={{ 'verticalAlign': 'middle' }}>
                                    ...
                                    <td>{room.rvComment !== null ? room.rvComment : '-'}</td>
                                    <td style={{ 'textAlign': 'center' }}>{room.rvScore != null ? room.rvScore : '-'}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
